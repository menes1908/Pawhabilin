                ...
                <div class="space-y-3">
                    <div>
                        ...
                        <div class="flex items-center gap-2 mb-2">
                            ...
                            <!-- Availability badge removed from here (will display next to price) -->
                        </div>
                    </div>
                                ...
                                <div class="filter-bar">
                                    ...
                                    <div class="sort-wrapper" style="display:flex;flex-wrap:wrap;gap:8px;align-items:center;">
                                        ...
                                        <select name="ptype" id="ptype-select">
                                            <?php
                                            $petTypes=[''=>'All','Dog'=>'Dog','Cat'=>'Cat','Bird'=>'Bird','Fish'=>'Fish','Reptile'=>'Reptile','Small Pet'=>'Small Pet','Other'=>'Other'];
                                            foreach($petTypes as $val=>$label){ $sel=$ptype===$val?'selected':''; echo '<option value="'.h($val).'" '.$sel.'>'.h($label).'</option>'; }
                                            ?>
                                        </select>
                                    </div>
                                </div>
                                ...
                                <div class="product-card" data-product-id="<?= (int)$p['products_id'] ?>" data-category="<?= h($p['products_category']) ?>" data-name="<?= strtolower(h($p['products_name'])) ?>">
                                    ...
                                    <div class="space-y-3">
                                        ...
                                        <div class="flex gap-2 items-stretch">
                                            ...
                                            <?php if($disabled): ?>
                                                <span class="inline-flex flex-1 items-center justify-center px-2 py-1 rounded-md bg-red-100 text-red-700 border border-red-300 text-[10px] font-semibold tracking-tight leading-none">
                                                    Unavailable
                                                </span>
                                            <?php else: ?>
                                                <form method="post" action="../../shop/cart_add.php" class="flex gap-2 ajax-cart-add" data-product-id="<?= (int)$p['products_id'] ?>" style="flex:1;">
        ...
        async function performSearch(openIfSingle=false){
            ...
            try{
                ...
                if(openIfSingle){
                    ...
                    if(cards.length===1){
                        const id=cards[0].getAttribute('data-product-id'); if(id) openQuickViewDb(id);
                    } else {
